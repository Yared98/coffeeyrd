...
use crate::db;
use crate::models::{
    AdminDeleteResponse, AdminMetricsResponse, AdminPurgeResponse,
};
...
pub fn get_configured_admin_token() -> String {
    std::env::var("ADMIN_TOKEN").unwrap_or_else(|_| "dev_admin_coffeeyrd_secret".to_string())
}
...
async fn login_handler(
        ...
        if attempts >= RATE_LIMIT_MAX_ATTEMPTS {
            if last_attempt.elapsed() < Duration::from_secs(RATE_LIMIT_WINDOW_SECS) {
                ...
                warn!(
                    ...
                    "Admin login bloqueado temporariamente por rate limit no CoffeeYrd"
                );
                return Err((
    ...
    if !is_valid {
        ...
        warn!(client_ip = %client_ip, "Tentativa de login administrativo com token inválido no CoffeeYrd");
    ...
    info!(client_ip = %client_ip, "Sessão de administrador iniciada com sucesso no CoffeeYrd");
...
async fn metrics_handler(
    ...
    let conn = Connection::open(&state.db_path).map_err(|e| {
        tracing::error!(error = %e, "Erro ao abrir conexão com banco para admin metrics");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let metrics = db::get_admin_metrics(&conn, &state.db_path).map_err(|e| {
        tracing::error!(error = %e, "Erro ao obter métricas de admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let sessions = db::list_admin_sessions(&conn, 50).map_err(|e| {
        tracing::error!(error = %e, "Erro ao listar sessões para admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let active_sessions_memory = state.sessions.len();
...
async fn purge_handler(
    ...
    let purged_count = db::cleanup_expired_sessions(&conn, retention_days).map_err(|e| {
        tracing::error!(error = %e, "Erro ao executar expurgo manual");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    info!(purged_count = purged_count, "Expurgo manual de sessões executado via Admin no CoffeeYrd");
...
async fn delete_session_handler(
    ...
    let count = db::delete_session_by_admin(&conn, &session_id).map_err(|e| {
        tracing::error!(error = %e, "Erro ao deletar sessão via admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    state.sessions.remove(&session_id);
    info!(session_id = %session_id, "Sessão removida pelo administrador");
...
mod tests {
    ...
    async fn test_admin_auth_and_rate_limit() {
        let state = AppState::new(":memory:".to_string());
