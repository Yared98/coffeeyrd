...
// Blindado para privacidade estrita:
// 1. O script é injetado dinamicamente APENAS se configurado via .env / backend /api/config.
// 2. data-auto-track="false" é sempre aplicado (impede envio automático de rotas com tokens).
// 3. Rotas de sessão são sempre mascaradas para '/room', sem IDs de reunião e sem tokens.
 ...
 * Garante que rotas de reunião não vazem o ID da sala nem o token do facilitador.
 */
export function trackPageView(sanitizedPath: '/' | '/room', title?: string) {
  try {
    if (typeof window !== 'undefined' && window.umami) {
      window.umami.track((props) => ({
        ...props,
        url: sanitizedPath,
        title: title || (sanitizedPath === '/' ? 'CoffeeYrd — Início' : 'CoffeeYrd — Reunião'),
      }));
    }
  } catch (err) {
